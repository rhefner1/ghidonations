...
function loadTeams() {
    ...
    var params = {
        action: 'pub_allTeams',
        arg0: JSON.stringify(settings_key)
    };
...
function showInfo(team_key, individual_key) {
    ...
    var params = {
        action: 'pub_individualInfo',
        arg0: JSON.stringify(team_key),
        arg1: JSON.stringify(individual_key)
    };
    rpcGet(params, function (data) {
        if (data != null) {
...
function submitForm() {
    ...
    if ($('#cover_trans').attr('checked') == 'checked') {
        ...
        var addition = 0.022 * new_price;
        new_price += addition;
        do {
            addition = 0.022 * addition;
            new_price += addition;
        } while (addition > 0);
        new_price += 0.31;
